...
describe('validateCamera', () => {
  ...
  it('requires a non-empty name', () => {
    ...
    expect(validateCamera({ name: '   ', source: { scheme: 'rtsp', host: 'x' } }).valid).toBe(false);
  });
  ...
  it('requires a host and rejects hosts with injection characters', () => {
    ...
    for (const host of ['has space', 'a/b', 'a@b', 'a?b', 'a\\b', 'a"b']) {
      expect(validateCamera({ name: 'c', source: { scheme: 'rtsp', host } }).valid, host).toBe(false);
    }
  });

  it('rejects out-of-range or non-integer ports', () => {
    expect(validateCamera({ name: 'c', source: { scheme: 'rtsp', host: 'x', port: 0 } }).valid).toBe(false);
    expect(validateCamera({ name: 'c', source: { scheme: 'rtsp', host: 'x', port: 70000 } }).valid).toBe(false);
    expect(validateCamera({ name: 'c', source: { scheme: 'rtsp', host: 'x', port: 55.5 } }).valid).toBe(false);
  });

  it('rejects a path with traversal or control characters', () => {
    expect(validateCamera({ name: 'c', source: { scheme: 'rtsp', host: 'x', path: '/a/../b' } }).valid).toBe(false);
    expect(validateCamera({ name: 'c', source: { scheme: 'rtsp', host: 'x', path: 'no-leading-slash' } }).valid).toBe(false);
    expect(validateCamera({ name: 'c', source: { scheme: 'rtsp', host: 'x', path: '/a b' } }).valid).toBe(false);
  });

  it('rejects credentials embedded in the camera definition', () => {
    const r = validateCamera({ name: 'c', password: 'secret', source: { scheme: 'rtsp', host: 'x' } });
