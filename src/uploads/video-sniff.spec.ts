...
describe("sniffVideoType", () => {
  it("accepts an ISO-BMFF mp4 (ftyp + isom brand) as video/mp4", () => {
    expect(
      sniffVideoType(
        bytes(SIZE_BOX, "ftypisom", [0, 0, 2, 0], "isomiso2avc1mp41"),
      ),
    ).toEqual({
  ...
  it("accepts a WebM file (EBML + webm doctype) as video/webm", () => {
    expect(
      sniffVideoType(
        bytes([0x1a, 0x45, 0xdf, 0xa3], "some ebml header webm more"),
      ),
    ).toEqual({
  ...
  it("accepts a Matroska file as video/x-matroska", () => {
    expect(
      sniffVideoType(
        bytes([0x1a, 0x45, 0xdf, 0xa3], "header matroska doctype"),
      ),
    ).toEqual({
  ...
  it("rejects HTML masquerading as a video", () => {
    expect(
      sniffVideoType(bytes("<!DOCTYPE html><script>alert(1)</script>")),
    ).toBeNull();
  });

  it("rejects an ftyp box with an unknown brand", () => {
    expect(
      sniffVideoType(bytes(SIZE_BOX, "ftypXXXX", [0, 0, 0, 0])),
    ).toBeNull();
  });
