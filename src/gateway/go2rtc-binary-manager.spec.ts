...
describe('Go2rtcBinaryManager', () => {
  ...
  it('downloads the binary when missing and makes it available', async () => {
    ...
    const fetchImpl = (async (url: string) => {
      calledUrl = url;
      return { ok: true, status: 200, arrayBuffer: async () => new TextEncoder().encode('BINARY').buffer };
    }) as unknown as typeof fetch;

    const mgr = new Go2rtcBinaryManager({ dataDir: dir, platform: 'linux', arch: 'x64', fetchImpl });
  ...
  it('uses an already-present (manually placed) binary without downloading', async () => {
    const mgr = new Go2rtcBinaryManager({ dataDir: dir, platform: 'linux', arch: 'x64', fetchImpl: fakeFetch('SHOULD-NOT-RUN') });
  ...
  it('throws on an unsupported platform', async () => {
    const mgr = new Go2rtcBinaryManager({ dataDir: dir, platform: 'sunos', arch: 'sparc', fetchImpl: fakeFetch('x') });
  ...
  it('rejects a download that fails SHA-256 verification', async () => {
    const mgr = new Go2rtcBinaryManager({
      dataDir: dir, platform: 'linux', arch: 'x64', fetchImpl: fakeFetch('TAMPERED'), expectedSha256: '0'.repeat(64)
    });
