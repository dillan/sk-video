...
import { DiscoveryService, ScanThrottledError, type DiscoveryProbe } from './discovery-service';
...
describe('DiscoveryService', () => {
  ...
  it('keeps going when one probe rejects', async () => {
    const svc = new DiscoveryService({
      probes: [() => Promise.reject(new Error('mdns blew up')), probeOf({ hostname: 'cam.local' })],
    });
    const found = await svc.scan();
