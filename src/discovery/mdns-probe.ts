...
export function parseMdnsRecords(records: IMdnsRecord[]): IRawDiscovery[] {
  ...
  for (const r of records) {
    ...
    const label = sanitizeDeviceString(
      r.name.split(`.${service}`)[0] || data.target,
    );
    out.push({
      hostname,
 ...
 */
export function createMdnsProbe(
  make: typeof makeMdns = makeMdns,
): DiscoveryProbe {
